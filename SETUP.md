...
## 6.5 증권사 잔고 자동 동기화 — API 키 등록

**개념.** 자산 입력 탭의 🏦 동기화는 한투(연금저축·ISA)·키움(국내·미국)·빗썸의 잔고를 API로 읽어와 수량·평단·예수금을 맞춘다. 각 증권사의 API 키가 필요한데, **키는 Cloudflare 대시보드가 아니라 앱의 설정 탭에서 등록**한다 — portfolio 데이터와 똑같이 로그인 이메일 귀속으로 KV(`user:<이메일>:broker:keys`)에 저장되므로, 사용자마다 자기 키·자기 계좌만 쓰고 서로 완전히 격리된다. 등록된 키는 어떤 API로도 원본을 다시 보여주지 않는다 (설정 탭에는 앞 4자 마스킹만 표시).

**키 발급처** (각 사이트에서 본인이 직접 발급).
...
**등록 방법.** 설정 탭 → "🔑 증권사 API 키" 카드에 값 붙여넣기 → 저장. 증권사별 부분 등록도 된다 (등록된 곳만 동기화됨). 키를 바꾸면 다시 저장하면 되고, 삭제 버튼은 전체 키와 토큰 캐시를 지운다.

**동작 범위.** 조회 전용 API만 호출한다(주문 불가). 퇴직연금(DC)·한투 금현물은 증권사 API가 공식적으로 지원하지 않아 수동 입력 유지.
