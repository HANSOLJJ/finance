...
개인용 자산 포트폴리오 추적 앱. **로그인 계정별로 데이터가 분리**되는 다중 사용자 구조지만, 현재는 운영자 1인이 쓴다 (구글 OAuth 앱이 테스트 모드 — 사용자 추가 방법은 아래 "사용자 추가" 절).
...
Cloudflare Access 정책은 Include: **Everyone**이지만, **구글 OAuth 앱이 테스트 모드**라 실제로는 ① 구글 콘솔의 **테스트 사용자 목록**에 있는 계정 ② 구글 프로젝트 소유자(운영자)만 로그인된다. 주소만 공유해서는 못 들어온다.

사용자 추가 = 구글 콘솔 → Google 인증 플랫폼 → 대상 → 테스트 사용자 **Add users** 에 지메일 추가(한도 100명) → 주소 공유 (docs/SETUP.md 3절). 새 사용자는 로그인 후 빈 포트폴리오에서 시작한다. 자리는 Zero Trust Free 50석이고 한 달 미접속 시 자동 회수된다. 차단은 목록에서 제거하면 된다.
