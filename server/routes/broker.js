// ============================================================================
// 증권사 잔고 조회 — GET /api/broker (functions/api/broker.js 이식판)
// 사용자가 등록한 연결(connections)을 순회하며 각 provider 어댑터로 잔고를 조회하고,
// 공통 형식으로 정규화해 돌려준다. 클라이언트 상대는 js/broker.js(🏦 동기화 모달).
// [보안 모델]
//  - Access JWT 검증(requireAuth)으로 로그인 이메일을 얻는다.
//  - 자격증명은 그 이메일 귀속 broker_connection 행에서 복호화해 로드(db.getConnections)
//    (등록은 /api/broker-connections — portfolio 와 동일한 per-user 규약이라
//    사용자 간 구조적으로 격리되고, 친구도 자기 연결을 등록하면 자기 계좌만 본다).
//  - 접근 토큰은 연결 단위로 broker_token 에 23시간 캐시 (lib/providers.js).
//  - 조회 TR만 호출한다.
...
async function fetchConnection(db, email, conn) {
...
export default function brokerRoutes(db) {
  const r = Router();
  r.use(requireAuth);
  ...
  r.get('/', async (req, res) => {
    ...
    res.json({ ok: true, fetchedAt: new Date().toISOString(), sources });
  });
