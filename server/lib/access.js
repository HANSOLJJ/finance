// Cloudflare Access JWT 검증 — functions/_lib/access.js 이식판 (Express req + 로컬 DEV_EMAIL 우회).
// Access 는 로그인 통과 요청에 Cf-Access-Jwt-Assertion 헤더(JWT)를 붙여주며, Tunnel 을 지나도 오리진까지
// 그대로 전달된다. 위조를 막기 위해 팀 공개키(RS256)로 서명을 검증한 뒤 email 클레임을 쓴다.
...
// 공개키 캐시 — 프로세스가 살아있는 동안 재사용해 요청마다 certs 조회를 피한다.
...
export async function getVerifiedEmail(req) {
  // 로컬 개발 우회 — 로컬엔 Access 헤더가 없어 전부 401 이므로 .env 의 DEV_EMAIL 을 그대로 쓴다.
  // 운영(Tunnel 뒤)에서는 절대 설정하지 않는다: 로그인 없이 임의 이메일로 남의 데이터에 접근 가능.
  if (process.env.DEV_EMAIL) return process.env.DEV_EMAIL;
  try {
...
// 인증 필수 라우트 공용 미들웨어 — 검증된 이메일을 req.email 에 싣고, 실패 시 원본과 같은 401 평문.
export async function requireAuth(req, res, next) {
  const email = await getVerifiedEmail(req);
  if (!email) return res.status(401).type('text/plain').send('unauthenticated');
  req.email = email;
  next();
}
