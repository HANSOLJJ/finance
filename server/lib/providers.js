...
// 새 증권사 추가 = 이 파일에 항목 1개 + lib/brokers.js 에 정규화 함수 1개.
// 저장 구조·설정 UI·diff·미리보기는 이 선언을 읽어 동작하므로 수정할 필요가 없다.
// (functions/_lib/providers.js 이식판 — 토큰 캐시가 KV 에서 SQLite(ctx.db)로 바뀐 것 외 동일.)
//
// [provider 항목 스펙]
//  label            표시 이름
...
async function cachedToken(ctx, issue, force = false) {
  if (!force) {
    const cached = ctx.db.getToken(ctx.email, ctx.connId);
    if (cached) return cached;
  }
  const token = await issue();
  ctx.db.putToken(ctx.email, ctx.connId, token);
  return token;
}
...
// (토스증권은 2026-05 개인 API 출시, 호출 IP allowlist 요구 — Mac mini 고정 IP 환경이면 붙일 수 있다. 후속 과제.
//  삼성증권은 기관 전용이라 개인 연동 불가.)
// ---------------------------------------------------------------------------
